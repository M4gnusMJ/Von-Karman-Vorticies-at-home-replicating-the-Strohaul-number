...
- The cylinder speed was not quite constant in all runs, nor quite straight lined (i.e swayed from side to side)
- The additives to the water could have changed the fluids properties.
ect. After all, it was a bathtub.

Additionally, due to the minimal setup not too many runs were suitable for analysis - either the speeds were too higi, making the Von Karman vorticies hard to distinguish from other turbulence, or the speed was *really* uneven (stop/start motion from cylinder rubbing on the bathtub). After selecting based on the smoothness of the cylinder motion and the visibility of vorticies, we were left with 7 runs for further analysis.

### AI disclaimer
While I know how to write R code, I am not the best R programmer and neither is that the point of this document. Most of the code is written by GPT codex 3, and all code has been reviewed by me.

### Code and data analysis 
The velocity of the cylinder was found using the open source physics software Tracker, via calibrating size to the reference stick then using autotracking. The data was then exported is .csv format to .txt files. These are what we will be working on moving forwards. 
...
As such, the vortex counts should be treated as rough estimates at best. In spite of this, as we will see later, the results still appear aligned with the literature. 
...
### Correlation
As St should be fairly constant for Re, we would expect a Pearsons correlation coefficient close to 0. We will do this with and without the outlier (red dot in above plot). (*Note, cor.test also calculates p values ect*)
```{r statistics}
...
Key point is that without the outlier, we get a correlation close to 0. Also note that for both calculations we do not have the statistical power to say that either correlation is meaningfully (p < 0.05) different from 0.

### Ensemble averaging
As the Strohaul Number in the irregular range ($300 < Re < 10^4$) is reported to be approximately constant compared to Re (Roshko, 1954), we can meaningfully take the ensemble average over all runs to approximate the actual St. As with the previous cell, this will be done with and without the outlier.
...
**Assuming my runs are representative of the underlying population**, we can use bootstrapping with the boot library in R to estimate the confidence interval.
...
par(old_par)
```
Based on the data in Roshko (1954), we expect a St number around 0.21, which is within the 95% confidence interval when we include the outlier, and just outside the confidence interval without the outlier (*note: but not outside both if confidence intervals are found assuming an underlying normal distribution*). 

A final note on the distributions: As expected due the law of large numbers, both distributions appear approximately normal, but interestingly we see some skewness in the distribution that includes the outlier - perhaps indicative that this is truly an outlier, i.e the belongs to a different population than the other runs.
